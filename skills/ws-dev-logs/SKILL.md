...
Use this skill inside a git worktree managed by `ws-dev` to navigate crash logs, search for errors, and reset logs before retrying, via the four MCP tools that `ws-dev mcp` exposes.

## Prerequisites

The MCP server must be registered in the worktree's `.claude/settings.local.json`, pointing at the `ws-dev` binary with `cwd` set to the worktree directory. If `list_logs`, `tail_log`, `truncate_log`, and `search_log` are not visible, ask the user to configure the MCP server per the `## MCP` section of the repo's README and retry.

The server runs in the worktree's current working directory. Its log directory resolves in this order: `--log-dir` flag > `$WS_DEV_LOG_DIR` > `log`.
...
- **`tail_log`** — Last N lines of `<name>.log`. Default `lines: 100`. Bump to 500–1000 when chasing a stack trace or when the user says "give me more". This is a snapshot of the current tail, not a live follow — for live following, point the user at the terminal command `ws-dev logs <worktree> <name> -f`.
...
- **`truncate_log`** — **Destructive. In-place. No backup. Not recoverable.** Only call when the user has explicitly asked to clear a log (e.g. "wipe the web log and retry"). Never call pre-emptively or as cleanup. After truncating, tell the user to restart the process with `ws-dev server <worktree>` — the MCP server does not restart processes.
...
#   "Restart the process with `ws-dev server <worktree>` and reproduce the issue."
```
...
- **Scope is one worktree.** The MCP tools only see the log directory of the worktree they run in. For cross-worktree comparisons, the user must run Claude inside each worktree separately.
- **No live follow.** `tail_log` returns a snapshot. For continuous tailing, the terminal command `ws-dev logs <worktree> <name> -f` is the right answer.
